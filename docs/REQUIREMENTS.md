...
- Testcontainers-based integration suites cover Contact/Task/Appointment services against real Postgres, while new JWT/config/filter/unit tests bring the total to 920 test executions.
...
- Latest CI: 920 tests passing (unit + slice + Testcontainers + security + filter + config + Project/Task/Appointment enhancement tests), ~83% mutation score, 96%+ line coverage on stores, 95%+ on mappers, SpotBugs clean.
